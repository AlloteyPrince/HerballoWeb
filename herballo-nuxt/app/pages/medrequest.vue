<template>
  <div class="medrequest-page">
    ...
    <div v-if="!formVisible" class="intro-screen">
      ...
      <div class="hero">
        <div class="hero-content">
          ...
          <div class="hero-badge">
            <span>🩺</span> Free for all healthcare professionals in Ghana
          </div>

          <!-- Main title -->
          <h1 class="hero-title">MedRequest</h1>
          <p class="hero-sub">Generate lab and scan requests in a few clicks</p>

          <!-- Live counter -->
          <p v-if="exportCount > 0" class="hero-counter">
            ✦ {{ exportCount.toLocaleString() }} request form{{ exportCount === 1 ? '' : 's' }} generated and counting
          </p>

          <!-- Buttons -->
          <div class="hero-actions">
            <button class="hero-btn-outline" @click="showHowTo = true">
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                <circle cx="12" cy="12" r="10"/><path d="M9.09 9a3 3 0 0 1 5.83 1c0 2-3 3-3 3"/><line x1="12" y1="17" x2="12.01" y2="17"/>
              </svg>
              How to use MedRequest
            </button>
            <button class="hero-btn-solid" :class="{ loading: loadingForm }" @click="proceedToForm" :disabled="loadingForm">
              <span v-if="!loadingForm" class="btn-content">
          ...
          <div class="hero-footer-row">
            <button class="share-link-btn" @click="shareLink">
            ...
            <span class="hero-trust">Powered by Herballo · herballo.co</span>
          </div>
      ...
      <!-- HOW TO MODAL -->
      <Teleport to="body">
        <div v-if="showHowTo" class="modal-overlay" @click.self="showHowTo = false">
          <div class="modal">
            <div class="modal-header">
              <h2 class="modal-title">How to use MedRequest</h2>
              <button class="modal-close" @click="showHowTo = false" aria-label="Close">
            ...
            <div class="modal-body">
              <ol class="howto-steps">
                <li>
                  <strong>Click "Generate Request Form"</strong> — This takes you to the form where you will fill in all the details for the lab request.
                </li>
                <li>
            ...
            <div class="modal-footer">
              <button class="modal-proceed-btn" :class="{ loading: loadingForm }" @click="showHowTo = false; proceedToForm()" :disabled="loadingForm">
                <span v-if="!loadingForm" class="btn-content">
                  Generate Request Form
    ...
    <div v-if="formVisible">
      <div class="form-banner">
        <img src="/images/medrequest-hero.jpg" alt="MedRequest by Herballo" class="form-banner-img" />
        <div class="form-banner-overlay">
          <h2 class="form-banner-title">MedRequest</h2>
          <p class="form-banner-sub">Generate a professional lab request form in seconds — free for all doctors</p>
        </div>
      </div>
...
useHead({
  ...
  meta: [
    ...
    { property: 'og:image', content: 'https://herballo.co/images/medrequest-hero.jpg' },
    ...
    { name: 'twitter:image', content: 'https://herballo.co/images/medrequest-hero.jpg' },
  ],
...
async function exportPDF() {
  ...
  function tableSection(title, rows) {
    ...
    rows.forEach((cols, i) => {
      ...
      if (i < rows.length - 1) { doc.setDrawColor(210, 232, 220); doc.setLineWidth(0.2); doc.line(bx, ry + rowH, bx + bw, ry + rowH) }
      if (cols.length === 2) { doc.setDrawColor(210, 232, 220); doc.setLineWidth(0.2); doc.line(bx + bw / 2, ry, bx + bw / 2, ry + rowH) }
...
.hero {
  min-height: 100vh;
  background-image: url('/images/medrequest-hero.jpg');
...
.hero-counter {
  font-size: 13px;
  color: #0f6e56;
  background: rgba(228, 243, 236, 0.9);
...
.hero-btn-solid:hover {
  background: #085041;
  box-shadow: 0 6px 24px rgba(15, 110, 86, 0.45);
...
.modal {
  ...
  box-shadow: 0 20px 60px rgba(10, 30, 20, 0.25);
...
.modal-header {
  ...
  border-bottom: 1px solid #e1f2eb;
}
...
.modal-close:hover { background: #e1f2eb; color: #0f6e56; }
...
.howto-steps li strong { color: #0a3c28; }
...
.modal-proceed-btn:hover { background: #085041; }
...
.form-banner-overlay {
  position: absolute;
  inset: 0;
  background: linear-gradient(transparent 10%, rgba(10, 60, 42, 0.55) 60%, rgba(10, 60, 42, 0.75) 100%);
  display: flex;
  flex-direction: column;
  align-items: center;
  justify-content: center;
...
.hero-btn-solid.loading {
  background: #085041;
  cursor: not-allowed;
  opacity: 0.9;
}

.modal-proceed-btn.loading {
  background: #085041;
  cursor: not-allowed;
  opacity: 0.9;
}
...
.section-label { font-size: 11px; font-weight: 700; color: #0f6e56; text-transform: uppercase; letter-spacing: 0.07em; margin: 0 0 18px; }
...
.field input, .field select, .field textarea {
  font-size: 14px; color: #1a2e24; background: #f4f8f6;
  border: 1.5px solid #e1f2eb; border-radius: 10px; padding: 10px 14px;
...
.field input.error, .field select.error { border-color: #e24b4a; background: #fff8f8; }
.field textarea { resize: vertical; min-height: 90px; line-height: 1.6; }

.toggle-row { display: flex; gap: 12px; }

.toggle-btn {
  flex: 1; padding: 12px 16px; border: 1.5px solid #e1f2eb; border-radius: 12px;
  background: #f4f8f6; color: #3d5c4a; font-size: 14px; font-family: inherit;
  cursor: pointer; display: flex; align-items: center; justify-content: center;
  gap: 8px; transition: all 0.15s; font-weight: 600;
}
...
.count-badge {
  background: #0f6e56; color: white; font-size: 11px; font-weight: 700;
...
.search-icon { position: absolute; left: 12px; top: 50%; transform: translateY(-50%); color: #8aaa97; pointer-events: none; }

.search-input {
  font-size: 14px; color: #1a2e24; background: #f4f8f6;
  border: 1.5px solid #e1f2eb; border-radius: 10px;
...
.item-list { max-height: 220px; overflow-y: auto; border: 1.5px solid #e1f2eb; border-radius: 12px; background: white; }

.list-item {
  display: flex; align-items: center; gap: 10px; padding: 10px 16px;
  font-size: 14px; color: #1a2e24; border-bottom: 1px solid #f0f7f3;
...
.tag {
  ...
  padding: 5px 10px 5px 13px; background: #e4f3ec;
  border: 1px solid #9fd8c0; border-radius: 20px;
  font-size: 12px; color: #0a4a38; font-weight: 600;
}
...
.export-btn {
  ...
  font-size: 16px; font-weight: 700; font-family: inherit; cursor: not-allowed;
  display: flex; align-items: center; justify-content: center; gap: 10px;
  transition: all 0.2s; background: #d0e8dc; color: #6aaa8a;
}
...
@media (max-width: 600px) {
  ...
  .hero-btn-outline, .hero-btn-solid { justify-content: center; }
  .grid-2 { grid-template-columns: 1fr; }
  .toggle-row { flex-direction: column; }
  .card { padding: 18px 16px; border-radius: 14px; }
  .form-banner { height: 140px; }
  .modal { margin: 0; border-radius: 20px 20px 0 0; max-height: 90vh; }
